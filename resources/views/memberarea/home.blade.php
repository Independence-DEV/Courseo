                    ...
                    <div x-data="{ open: false }">
                        ...
                        <div x-show="open" class="bg-gray-100">
                            @foreach($courses as $course)
                                <a class="py-2 px-16 block text-sm text-gray-600 hover:bg-blue-500 hover:text-white" href="{{ route('account.memberarea.course', ['domain' => $account->subdomain, 'slug' => $course->slug]) }}">{{ $course->title }}</a>
                            @endforeach
                        </div>
                    </div>
                </nav>
